...
原專題把下降的濃度趨勢直接讀成減量成效。一條下降的線有兩個互相競爭的解釋，
...
- **不是因果歸因。** 它移除的是**模型看得見的**氣象影響。本地風速與濕度捕捉不到的
  境外傳輸會留在正規化序列裡，並被讀成排放改變。這就是每條趨勢旁邊都要報 R² 的理由：
  **解釋力低的模型不可能移除得多。**
...
也就是說：**觀測到的下降有四成二不是排放減少**，是天氣。按空品區看仍然成立
...
正規化回答了「趨勢裡有多少是天氣」，但沒有回答「某一個政策有沒有效」。
第二個問題由 [src/twair/analysis/events.py](../src/twair/analysis/events.py) 回答，
而它給的是**陰性結果，並且量出了那個陰性有多硬**：對每個事件，
同一套方法在**沒有事件的年份**跑一遍，得到安慰劑分布。

| 事件 | 站數 | 效應中位數 | 安慰劑均值 | 安慰劑 SD | 通過門檻 | 機率預期 |
|---|---|---|---|---|---|---|
| COVID-19 全國三級警戒 | 73 | −0.494 | −0.690 | 2.503 | 1 | 3.3 |
| 台中電廠生煤許可爭議 | 72 | −1.612 | +0.743 | 3.512 | 1 | 3.3 |
| 2018 空污法修正 | 72 | +0.454 | +0.024 | 0.660 | 1 | 3.3 |

三個事件的實際通過數都**低於**機率預期。COVID 那一列是關鍵：
安慰劑均值本身就是 −0.690，**方法在沒有封城的年份平均也會給出接近的下降**。
所以誠實的說法是「**測不到**」，不是「等於零」——這是方法的偵測極限，
不是事件的效應大小。1,295 與 1,208 個安慰劑估計值逐一列在網站第五章的表格裡。
...
原專題以 SAS Enterprise Guide / SPSS 的點選介面操作。點選不會留下紀錄：
