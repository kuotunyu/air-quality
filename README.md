...
2. **經驗模型與方法學代價量化**：針對台灣空品研究常見之月平均壓縮、逐步篩選 OLS 及「以 PM10 預測 PM2.5」等經驗做法，在同一組觀測資料上平行配適對照模型，精確量測各建模選擇的統計偏誤與資訊洩漏代價。
...
為量化傳統經驗建模之影響，本研究透過 `analysis/baseline.py` 實際配適對照基準模型，於同一組觀測數據上平行評估各項建模假設之統計代價：
...
> **方法學邊界宣告**：本階段評估僅量測機器學習模型於留出資料集（Held-out Sets）之增量預測價值（Predictive Value），非因果歸因推論、非感測器空間融合（Sensor Fusion），亦非直接以衛星反演地表 PM2.5 濃度。

| 資訊來源 | 驗證樣本規模 | 留出檢驗改善率 (Held-out) | 預測效益中位數<br/>(ΔRMSE · ΔR²) | 邊界與限制宣告 |
|---|---|---|---|---|
| **衛星遙測**<br/>(S5P + MAIAC) | 851 筆完整站月<br/>(76 測站 · 12 個月) | - 留出季度：3／4 fold 改善<br/>- 留出測站：9／10 fold 改善<br/>- 聯合轉移：37／40 fold 改善<br/>(全特徵綜合：49／54 fold) | ΔRMSE：−0.588<br/>ΔR²：+0.147 | 非因果推論、非衛星反演<br/>(校正與融合仍延後) |
| **ERA5 再分析**<br/>(BLH + 地表氣象) | 674,520 筆站時<br/>(77 測站 · 6 變數零缺失) | - 74 站共同樣本：205／222 fold 改善<br/>- 跨年同站：63／74 站改善<br/>- 跨年留站：70／74 站改善 | ΔRMSE：−0.758<br/>ΔR²：+0.249 | 尚未納入已發布 M4<br/>(正規化仍以站內觀測為準) |
