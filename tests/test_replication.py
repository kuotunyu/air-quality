...
def _full_month(
    station: str, year: int, month: int, values: dict[str, float]
) -> list[tuple[str, str, datetime, float | None, str]]:
    rows: list[tuple[str, str, datetime, float | None, str]] = []
...
class TestNaiveAggregation:
    ...
    def test_a_store_missing_a_variable_entirely_is_an_error(self, tmp_path: Path) -> None:
